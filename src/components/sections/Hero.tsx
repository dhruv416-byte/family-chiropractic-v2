...
const annotations: Annotation[] = [
  {
    ...
    show: 0.14,
    hide: 0.34,
    kicker: "01 · Realignment",
    title: "Restore the structure.",
    body: "Targeted spinal corrections release tension and bring the body back into balance — without force or guesswork.",
  },
  {
    id: "card-2",
    show: 0.38,
    hide: 0.6,
    kicker: "02 · Recovery",
    title: "Heal what's been holding you back.",
    body: "Soft-tissue work, posture re-education, and personalized at-home protocols to lock in the gains.",
  },
  {
    id: "card-3",
    show: 0.64,
    hide: 0.9,
    kicker: "03 · Reclaim",
    title: "Move like yourself again.",
    body: "Maintenance care that keeps every member of the family moving freely — for the long haul.",
  },
];
...
export default function Hero() {
  ...
  // Preload all frames with a progress bar
  useEffect(() => {
    let loadedCount = 0;
  ...
  // Draw a frame with cover-fit + DPR scaling
  const drawFrame = (index: number) => {
    ...
    // mobile zoom 1.15x for stronger subject framing on small screens
  ...
  // Scroll handler — RAF + ticking guard, direct DOM updates
  useEffect(() => {
    if (!loaded) return;
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        {/* hero text (fades out on first 8% of scroll) */}
        <div
          ref={heroTextRef}
        ...
        >
          ...
          <p className="mt-6 max-w-[48ch] text-base leading-relaxed text-white/70 md:text-lg">
            Care for every spine in the household — from your newest arrival to
        ...
        <div className="pointer-events-none absolute inset-0 hidden md:block">
          {annotations.map((card) => {
            ...
            return (
              <div
                key={card.id}
                className={`absolute right-10 top-1/2 max-w-[340px] -translate-y-1/2 transition-all duration-500 ${
                  visible
                    ? "translate-x-0 opacity-100"
                    : "translate-x-6 opacity-0"
                }`}
              >
                <div className="card-surface bg-white/95 p-6 backdrop-blur-md">
                  <div className="mb-3 text-[10px] font-medium uppercase tracking-wider text-indigo-500">
                    {card.kicker}
                  </div>
                  <h3 className="mb-2 text-lg font-semibold tracking-tight text-zinc-950">
                    {card.title}
                  </h3>
